...
import CandidatValidation from './candidatPayment';
...
function BankCandidat({token}:props) {
  ...
  const { data } = useQuery({ queryKey: ['bank_candidats',id], queryFn: ()=>getCandidatPaymentStatus(id as string) })
