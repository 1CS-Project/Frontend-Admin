import { bank } from "@/app/action";
import { BankT, addBank } from "@/app/mutations";
...
function AddBank({baladias,token,setShow}:props) {

  const queryClient=getQueryClient();

  const {mutate}=useMutation({
    mutationFn:(d:BankT)=>addBank(token,d),
    onSuccess:()=>{
      console.log("success");
      return queryClient.invalidateQueries({queryKey:["banks"]});
    }
  })
    ...
    return ( 
      <form onSubmit={(e)=>{
          ...
          const data={
            BanqueEmail:formData.get("email") as string,
            BanqueName:formData.get("name") as string,
            BaldyaSelect:baladias,
